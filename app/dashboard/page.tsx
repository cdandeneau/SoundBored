...
export default function DashboardPage() {
  ...
  const [selectedAlbum, setSelectedAlbum] = useState<SpotifyAlbumResult | null>(
    null
  );
  ...
  function addAlbumLocal(newAlbum: FavoriteAlbum) {
    setFavoriteAlbums((prev) => {
      const withoutSamePosition = prev.filter(
        (album) => album.position !== newAlbum.position
      );
      return [...withoutSamePosition, newAlbum].sort(
        (a, b) => a.position - b.position
      );
    });
  }

  function addTrackLocal(newTrack: FavoriteTrack) {
    setFavoriteTracks((prev) => {
      const withoutSamePosition = prev.filter(
        (track) => track.position !== newTrack.position
      );
      return [...withoutSamePosition, newTrack].sort(
        (a, b) => a.position - b.position
      );
    });
  }
  ...
  async function handleSaveBio(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!currentUserId) {
      setBioMessage("No logged-in user found.");
      return;
    }
                    ...
                    {trackResults.map((track) => (
                      <button
                        ...
                        className={`flex w-full items-center gap-3 rounded-lg p-3 text-left transition ${
                          selectedTrack?.spotify_track_id ===
                  ...
                  <div className="max-h-72 space-y-3 overflow-y-auto rounded-xl border border-zinc-800 bg-zinc-950/50 p-3">
                    {albumResults.map((album) => (
                      <button
                        ...
                        className={`flex w-full items-center gap-3 rounded-lg p-3 text-left transition ${
                          selectedAlbum?.spotify_album_id ===
